...
docker build -t ghcr.io/yox2yox/claude-remote:latest .
docker push ghcr.io/yox2yox/claude-remote:latest
```

プライベートレジストリを使う場合は `claude-remote.yaml` の `image:` を書き換え、
必要なら `imagePullSecrets` を追加してください。
...
Dockerfile の `@anthropic-ai/claude-code@latest` を特定バージョンに固定 or 再ビルド。
